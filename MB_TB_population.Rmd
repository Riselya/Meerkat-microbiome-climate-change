...
# 3. Fig SX. Visualising raw data per individual
...
# 6. Fig SX. Split by storage, season and group
...
top_groups<-group_freq$Var1[1:10]
...
# Fig 2a: Microbiome composition by year barplot
...
ord_plot<-ggplot(vectors_df, aes(x = CAP1, y = CAP2))+
 ...
 guides(fill = guide_legend(override.aes = list(size = 7)))+
  ...
  # add species
  geom_point(data = species_scores,  size = 4, col = "black", pch = 8, stroke = 1)+
...
ggarrange(ord_plot,year_plot,  ncol = 2 , common.legend = T, legend = "right", widths = c(4,2), labels = c("a)", "b)"))
...
sjPlot::plot_model(fit_genus, axis.lim = c(0.8,20), vline.color = "darkgrey", sort.est = T)+  theme_bw(base_size = 16)+ggtitle("")
...
genus_ps<-tax_glom(meerkat_merged_year, taxrank = "Genus")
...
top_genera_melt<-psmelt(top_genus_ps)
head(top_genera_melt)
...
taxanames<-c("Bacteroides", "Fusobacterium", "Alloprevotella", "Helicobacter","Parabacteroides","Enterococcus","Collinsella", "Pediococcus", "Lactococcus","Peptoclostridium")
...
taxa_df<-subset(top_genera_melt, Genus == taxanames[[i]])
#taxa_df<-subset(top_genera_melt, Genus == "Bacteroides")

head(taxa_df)

taxa_df<-taxa_df[,c(10,2,3)]

str(taxa_df)
names(taxa_df)[2]<-"Year"
taxa_df$Year<-as.integer(taxa_df$Year)
taxa_df<-taxa_df %>% arrange(Year)

ggplot(taxa_df, aes(x = Year, y = log10(Abundance)))+geom_point()+geom_path()

head(TB_summary_year)
head(yearly_temp_df)
head(yearly_rainfall_df)

TB_exposed_df<-subset(TB_summary_year, TB_status == "TB exposed")
TB_exposed_df<-TB_exposed_df[,c(1,3,5)]
names(TB_exposed_df)[2]<-"No_exposed"
names(TB_exposed_df)[3]<-"Prop_exposed"

TB_diseased_df<-subset(TB_summary_year, TB_status == "TB diseased")
TB_diseased_df<-TB_diseased_df[,c(1,3,5)]
names(TB_diseased_df)[2]<-"No_diseased"
names(TB_diseased_df)[3]<-"Prop_diseased"

max_temp_df<-yearly_temp_df[,1:2]
names(max_temp_df)[2]<-"Max_temp"

rainfall_df<-yearly_rainfall_df[,1:2]
names(rainfall_df)[2]<-"Rainfall"

#put all data frames into list
df_list <- list(taxa_df, TB_exposed_df, TB_diseased_df, max_temp_df, rainfall_df)

#merge all data frames in list
yearly_trends_df<-df_list %>% reduce(full_join, by='Year')

yearly_trends_df$Exposed_and_diseased<-yearly_trends_df$Prop_diseased+yearly_trends_df$Prop_exposed


yearly_trends_df<-subset(yearly_trends_df, Year>1998)

yearly_trends_df<-yearly_trends_df %>% mutate(temp_lag = lag(Max_temp))
yearly_trends_df<-yearly_trends_df %>% mutate(rainfall_lag = lag(Rainfall))
yearly_trends_df<-yearly_trends_df %>% mutate(TB_lag = lag(Prop_diseased))

head(yearly_trends_df)
...
yearly_trends_df<-na.omit(yearly_trends_df)

#global_model<- lm(log10(Abundance+1) ~ Prop_diseased + Prop_exposed + Max_temp + Rainfall, data = yearly_trends_df)

global_model<- lm(log10(Abundance+1) ~  Prop_diseased + Max_temp +Rainfall+ temp_lag + rainfall_lag, data = yearly_trends_df)

dredge_df<- MuMIn::dredge(global_model, m.lim = c(0,2))

head(dredge_df)
plot(dredge_df)


confset.95p <- get.models(dredge_df, cumsum(weight) <= .95)
avgmod.95p <- model.avg(confset.95p)
sum<-summary(avgmod.95p)
ci_df<-confint(avgmod.95p)


results<- data.frame(sum$coefmat.subset)

results<-cbind(results, ci_df)

names(results)[5]<-"P_val"
names(results)[6]<-"lower"
names(results)[7]<-"upper"

results$Taxa<-taxanames[i]
results$Variable<- row.names(results)


m.avg.list[[i]]<-results

}

yearly_results<-do.call(rbind,m.avg.list)

yearly_results$Sig<-yearly_results$P_val<0.01

head(yearly_results)

yearly_results$Taxa<-factor(yearly_results$Taxa, levels = rev(taxanames))

ggplot(yearly_results, aes(y = Taxa, x = Estimate))+
  geom_point(aes(fill = Sig), size = 3, pch = 21) + 
  geom_errorbarh(aes(xmin = lower, xmax = upper, col = Sig), height = 0)+
  facet_grid(~Variable, scales = "free_x")+
  geom_vline(xintercept = 0, linetype = "dashed")+
  ggthemes::theme_clean()+
  scale_fill_manual(values = c("darkgrey", "red"))+
  scale_color_manual(values = c("darkgrey", "red"))+
  labs(fill = "Significant", col = "Significant")
  




yearly_results2<-subset(yearly_results, Variable =="Prop_diseased" |   Variable =="Max_temp" |Variable =="Rainfall"|Variable == "temp_lag"|Variable == "rainfall_lag")

yearly_results2$Variable<-factor(yearly_results2$Variable, levels = c("Prop_diseased",  "Max_temp", "temp_lag", "Rainfall", "rainfall_lag"))

levels(yearly_results2$Variable)<-c("TB disease \nprevelance", "Max temp.", "Max temp. \n(lag)", "Rainfall", "Rainfall \n(lag)")

plot_s4<-ggplot(yearly_results2, aes(y = Taxa, x = Estimate))+
  geom_point(aes(fill = Sig), size = 3, pch = 21) + 
  geom_errorbarh(aes(xmin = lower, xmax = upper, col = Sig), height = 0)+
  facet_grid(~Variable, scales = "free_x")+
  geom_vline(xintercept = 0, linetype = "dashed")+
...
#put all data frames into list
df_list <- list(taxa_df, TB_diseased_df)

#merge all data frames in list
bacteroides_df<-df_list %>% reduce(full_join, by='Year')


   bacteroides_plot<- ggplot(bacteroides_df, aes(y = Prop_diseased, x = log10(Abundance)))+
  geom_point(size = 3, pch = 21, fill = "grey") + 
 theme_bw()+
  geom_smooth(method = "lm", col = "black")+
     ylab("TB disease prevalence")+
     xlab("Bacteroides count (log10)")
  
  ggarrange(plot_s4, bacteroides_plot, nrow = 1, widths = c(2,1), common.legend = T, legend = "bottom", labels = c("a)", "b)"))
...
P1<- ggplot(class_per_sample, aes(y = Abundance, x = reorder(Sample, Storage), fill = Class_plot))+
  facet_grid(~IndividID, scales = "free_x")+
  geom_col(position = "fill", stat="identity")+ 
  scale_fill_manual(values = rev(pal))+
  ylab("Relative abundance")+
  ...
  scale_x_discrete( labels = unique(class_per_sample$Storage))


P2<-ggplot(subset(class_per_sample, Class_plot != "Other"), aes(y = Abundance, x = Storage, fill = Class_plot, group = Storage))+
  facet_wrap(~Class_plot, scales = "free", ncol = 8)+
  geom_boxplot()+ 
  scale_fill_manual(values = rev(pal)[-3])+
  ylab("Relative abundance")+
  theme(axis.text.x = element_text(angle =45,  hjust=1), axis.title.x = element_blank())+
 #  scale_y_continuous(expand = c(0,0)) +
  labs(fill = "Bacterial class")+
...
ggarrange(P1, P2, ncol = 1, common.legend = T, legend = "right", labels = c("a)", "b)"))


### beta diversity

pilot_rare<-rarefy_even_depth(pilot, sample.size = 10000)
head(sample_data(pilot_rare))

## beta 

pilot.ord <- ordinate(pilot_rare, "NMDS", "wunifrac")
pilot.dist <- distance(pilot_rare, "wunifrac")


beta_plot<-plot_ordination(pilot_rare, pilot.ord, type="sample", color="Storage")+
  geom_line(aes(group = IndividID), col = "black")+
  geom_point(aes(fill = Storage), size = 4, pch = 21, col = "black")+
  scale_fill_manual(values  = c("skyblue", "red"))
  

metadata<- data.frame(sample_data(pilot_rare))

adonis(pilot.dist~Storage + IndividID, data = metadata )

#alpha


alpha_df<-estimate_richness(pilot_rare)

summary(alpha_df$Observed)
alpha_df$feature.id<-row.names(alpha_df)
metadata<-merge(metadata, alpha_df, by = "feature.id")

alpha_plot<-ggplot(metadata, aes(x = Storage, y = Shannon, fill = Storage))+
  geom_boxplot(alpha = 0.5)+
  geom_point( pch = 21, size = 3)+
   scale_fill_manual(values  = c("skyblue", "red"))+
  geom_line(aes(group = IndividID))

left_panel<-ggarrange(P1, P2, ncol = 1, common.legend = T, legend = "bottom", labels = c("a)", "b)"))

right_panel<-ggarrange(alpha_plot, beta_plot, ncol = 1, common.legend = T, legend = "bottom", labels = c("c)", "d)"))

ggarrange(left_panel, right_panel, widths = c(2,1), align = "h")
...
sample_data(pilot_clr)<-sample_data(pilot_clr)[,c("feature.id")]

class_per_sample<-psmelt(pilot_clr)
...
ggplot(subset(class_per_sample, Class_plot != "Other"), aes(y = Abundance, x = Storage, fill = Class_plot, group = Storage))+
  facet_wrap(~Class_plot, scales = "free", ncol = 8)+
  geom_boxplot()+ 
  scale_fill_manual(values = rev(pal)[-3])+
  ylab("Relative abundance")+
...
pilot.ord <- ordinate(pilot_rare, "MDS", "wunifrac")
pilot.dist <- distance(pilot_rare, "bray")

plot_ordination(pilot_rare, pilot.ord, type="sample", color="Storage", title="taxa")+
  geom_line(aes(group = IndividID), col = "black")+
  geom_point(size = 4)
metdata<- data.frame(sample_data(pilot_rare))
...
captive_meerkats<-ggplot(subset(class_per_sample, Storage == "FROZEN"), aes(y = Abundance, x = Sample, fill = Class_plot))+
  #facet_grid(~IndividID, scales = "free_x")+
  geom_col(position = "fill", stat="identity")+ 
  scale_fill_manual(values = rev(pal))+
  ylab("Relative abundance")+
   scale_y_continuous(expand = c(0,0)) +
  labs(fill = "Bacterial class")+
   theme(plot.margin=unit(c(1,0.2,0.2,0),"cm"))+
  theme(legend.justification = c("left", "center"))+
  theme(axis.title.x = element_blank(), axis.text.x = element_text(angle = 90))+
  theme(axis.title.y = element_blank(), axis.text.y = element_blank())

barplots<-ggarrange(plot_mb_year, captive_meerkats, ncol = 2, widths = c(2.2,0.8), common.legend = T, legend = F, align = "h", labels = c("    a) Wild", "b) Captive"), hjust = -0.1)

ord_plot2<-ord_plot+ theme(plot.margin=unit(c(1,0.2,0.5,0),"cm"))

ggarrange(barplots, ord_plot2, ncol = 2 , common.legend = T, legend = "right", widths = c(1,1.5), labels = c(NA, "c)"))


```

# Human and captivity effects

```{r}

allsamples_ps<-readRDS("phyloseq_May22_allsamples.RDS")

allsamples_ps

head(sample_data(allsamples_ps))
table(sample_data(allsamples_ps)$Species)
allsamples_ps<-subset_samples(allsamples_ps, Species == "Meerkat")

allsamples_core<-core(allsamples_ps, detection = 100, prevalence = 0)


#all_samples.ord <- ordinate(allsamples_core, "MDS", "wunifrac")
all_samples <- distance(allsamples_core, "wunifrac")

names(sample_data(allsamples_core))


plot_ordination(allsamples_core, all_samples.ord, type="sample", color="Species")+
  #geom_line(aes(group = IndividID), col = "black")+
  geom_point(aes(fill = Species, shape = Captive), size = 2,  col = "black")+
...
all_samples_bray.ord <- ordinate(allsamples_core, "NMDS", "bray")
#all_samples <- distance(allsamples_core, "wunifrac")

names(sample_data(allsamples_core))


plot_ordination(allsamples_core, all_samples_bray.ord, type="sample", color="Species")+
  #geom_line(aes(group = IndividID), col = "black")+
  geom_point(aes(fill = Captive), size = 2,  col = "black", pch = 21)
